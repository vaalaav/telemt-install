...
После любого изменения через `mytelemtinfo` запускается **краткая проверка работоспособности** — статус всех инстансов, VLESS-туннеля, привязки upstream'ов. Полный health check доступен в подменю «Управление прокси» → пункт 10.
